...
from train import multiscale_train
from utils import get_data_weighted
...
def run_experiment(config):
    ...
    model, v_losses, g_losses, losses = multiscale_train(
        ...
        use_supervised_prior=config.use_supervised_prior,
        ema=ema,
        device=device,
    )
    ...
    # 去除数据 scale prior 标注，用于 evaluation
    df = get_data_weighted(df)
